...
//   const { transactionId } = router.query;
  const getHistoryById=()=>{
    ...
    if (id) {
        dispatch(
          getHistoryByIdAction(transactionId,token)
        );
    }
  }
  ...
  const handleDownload = async ()=>{
    const token = JSON.parse(localStorage.getItem("token"));
    const id = router.query.transactionId ?? '' ;
    const response = await dispatch(downloadPdfAction(transactionId, token))
        ...
        <div>
          <div className={`${styles["history-section"]}`}>
            ...
            <div className={`${styles["trf-detail"]}`}>
              ...
              <p className={`${styles["detail-trf"]}`}>
                Rp. {userData.balance - data.amount}
              </p>
            </div>
            <div className={`${styles["trf-detail"]}`}>
            ...
            <div className="d-flex justify-content-end">
                <div className="d-flex mx-5">
                    ...
                    <button className={`${styles["btn-3"]}`}>Back To Home</button>
                </div>
            </div>
          </div>
        </div>
